  ...
  return (
    <div className=' relative space-y-2 flex flex-col w-[300px] h-[350px] select-none p-5 border border-purple-400 rounded-md '>
        ...
        <div className=" flex text-lg ">
            ...
            {
            quiz?.tags?.map((tag,i)=>(
                <div key={i} className=" px-1 mx-1 py-[1px]  max-w-max flex  items-center justify-center rounded-lg bg-yellow-100">{tag}</div>
            ))
        }
        </div>
        ...
        <div className=" absolute bottom-11 left-0 right-0  flex  justify-center  w-full">
            <Link to={quiz?._id}>
        <button className='border-blue-950  hover:bg-blue-500  border bg-blue-600 max-w-max rounded-md px-4 py-1'>Take Test</button>
            </Link>
        </div>
    </div>
  )
